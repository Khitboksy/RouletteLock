...
async function handleApi(request: Request): Promise<Response> {
  ...
  if (
    ...
    if (!body || !body.originalName)
      return errorResponse("originalName is required");
  ...
  if (
    ...
    return jsonResponse({
      branch,
      status: hasChanges ? status.split("\n") : [],
      hasChanges,
    });
  }
  ...
  if (
    ...
    try {
      ...
      if (!addResult.success)
        return errorResponse(`git add failed: ${addResult.stderr.toString()}`);
      ...
      if (!commitResult.success) {
        return errorResponse(
          `git commit failed: ${commitResult.stderr.toString()}`,
        );
      }

      return jsonResponse({
        message: commitResult.stdout.toString().trim(),
        committed: true,
      });
    } catch (err) {
...
console.log(`\n  🎲 RouletteLock Dev API running at http://localhost:${PORT}`);
console.log(`  🌐 Vite Developer Frontend: http://localhost:5173\n`);
